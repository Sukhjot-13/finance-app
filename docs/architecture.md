...
- **`src/app/(main)/layout.js`** - Main app shell with sidebar, header, and user context ("use client").
  ...
  - `MainLayout()` - Fetches user via `GET /api/user`. On transient failure shows an inline **retry card** instead of redirecting. Uses a locked viewport (`fixed inset-0 h-[100dvh]`) preventing iOS rubberband bounce; fixed non-scrolling header (`shrink-0 z-20 touch-none select-none`) with safe-area padding and touch-action suppression preventing downward scroll drag on the navbar; isolated content scrolling (`overscroll-y-none`); and touch gesture handler supporting edge-swipe right from the left screen boundary to open the mobile drawer. Route changes close the mobile sidebar via render-time adjustments.
